...
/// Whether the count total is fixed by the family or supplied by each row.
#[derive(Debug, Clone, Copy)]
pub(super) enum TrialPolicy {
    Fixed(u32),
    PerObservation,
}
...
pub(super) fn validated_total<const K: usize>(
    ...
    if !total.is_finite()
        || matches!(policy, TrialPolicy::Fixed(expected) if total != f64::from(expected))
    {
        return None;
    }
    Some(total)
...
pub(super) fn multinomial_nll_at_empirical_probabilities<const K: usize>(
    counts: &[f64; K],
    total: f64,
) -> f64 {
    if total == 0.0 {
    ...
    let mut nll = -ln_gamma_stirling_residual(total) - total.ln();
...
pub(super) fn negative_log_multinomial_coefficient<const K: usize>(
    counts: &[f64; K],
    total: f64,
) -> f64 {
    let empirical_nll = multinomial_nll_at_empirical_probabilities(counts, total);
    if total == 0.0 {
